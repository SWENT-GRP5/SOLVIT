...
Extremely well-built Figma, even incorporating a comprehensive design guideline.
...
As the sign-in fail, only 2 screens can be accessed. 
Rotation of the screen is possible but it breaks the UI
...
Note that some of your test are commented and that they rarely cover failure. 
It would be great to implement some error handling or add a check to ensure that the toasts are displayed.
Sadly, the "end-to-end" test is commented.... And it does not simulate users actions.
...
It is possible to double-tap the back arrow, which leads to a blank page.
...
Avoid having task in the product backlog that have a sprint label.
