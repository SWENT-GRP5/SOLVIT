...
class AuthRepositoryTest {
  ...
  fun testSignInWithGoogleSuccess() {
    ...
    `when`(mockAuth.signInWithCredential(any())).thenReturn(Tasks.forResult(mockTaskAuthResult))
    `when`(mockTaskAuthResult.user).thenReturn(mockUser)
    `when`(mockUser.uid).thenReturn("testUid")
  ...
  fun testSignInWithGoogleFailure() {
    ...
    `when`(mockGoogleSignInAccount.idToken).thenReturn("mockIdToken")
    `when`(mockAuth.signInWithCredential(any())).thenReturn(Tasks.forException(exception))
  ...
  fun testRegisterWithEmailAndPasswordSuccess() {
    ...
    authRepository.registerWithEmailAndPassword(
        "testRole", "test@example.com", "password", onSuccess, onFailure)
