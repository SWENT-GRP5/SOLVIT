...
class NotificationViewModelTest {
  ...
  fun init_fetchesNotifications() = runBlocking {
    ...
    `when`(notificationsRepository.getNotification(eq(providerId), any(), any())).thenAnswer {
        invocation ->
  ...
  fun sendNotifications_sendsAndFetchesUpdatedNotifications() = runBlocking {
    ...
    doAnswer { invocation ->
        ...
        .sendNotification(eq(serviceRequest), eq(matchingProviders), any(), any())
    ...
    doAnswer { invocation ->
        ...
        .`when`(notificationsRepository)
        .getNotification(eq(providerId), any(), any())
    ...
    notificationsViewModel.sendNotifications(
        serviceRequest, matchingProviders) // Calls getNotifications() again

    // Verify that getNotification was called twice: once in init() and once after sending
    // notifications
