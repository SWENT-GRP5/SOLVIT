...
class NotificationsRepositoryTest{
    ...
    private val serviceRequest = ServiceRequest("service123", "title", Services.TUTOR,"","","",Timestamp.now(),Timestamp.now(),null)
    ...
    fun `sendNotification sends notifications to matching providers`() {
        ...
        `when`(mockDocumentReference.set(any())).thenReturn(com.google.android.gms.tasks.Tasks.forResult(null))

        // Call the function to send notifications
        notificationsRepository.sendNotification(serviceRequest, matchingProvider, {
            // On success
    ...
    fun `documentToNotif converts document snapshot to Notification`() {
        ...
        `when`(mockDocumentSnapshot.getString("message")).thenReturn("A new service request has been posted.")
        `when`(mockDocumentSnapshot.getTimestamp("timestamp")).thenReturn(Timestamp.now())
        `when`(mockDocumentSnapshot.getBoolean("isRead")).thenReturn(false)
