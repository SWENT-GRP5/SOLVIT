...
class PackageProposalRepositoryFirestore(private val db: FirebaseFirestore) : PackageProposalRepository {
        ...
        override fun getPackageProposal(onSuccess: (List<PackageProposal>) -> Unit, onFailure: (Exception) -> Unit) {
            Log.d("PackageProposalRepositoryFirestore", "getPackageProposal")
            db.collection(collectionPath).get().addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    val proposal = task.result?.mapNotNull { document -> documentToPackageProposal(document) } ?: emptyList()
                    onSuccess(proposal)
                } else {
        ...
        override fun addPackageProposal(proposal: PackageProposal, onSuccess: () -> Unit, onFailure: (Exception) -> Unit) {
            performFirestoreOperation(
                db.collection(collectionPath).document(proposal.uid).set(proposal), onSuccess, onFailure)
        }

        override fun updatePackageProposal(proposal: PackageProposal, onSuccess: () -> Unit, onFailure: (Exception) -> Unit) {
            performFirestoreOperation(
                db.collection(collectionPath).document(proposal.uid).set(proposal), onSuccess, onFailure)
        }

        override fun deletePackageProposal(id: String, onSuccess: () -> Unit, onFailure: (Exception) -> Unit) {
            performFirestoreOperation(
