...
@Composable
fun TestChatScreen(chatAssistantViewModel: ChatAssistantViewModel, navigationActions: NavigationActions) {

    val messagesState = MutableStateFlow<List<ChatMessage.TextMessage>>(emptyList())
    ...
    Scaffold(
        ...
        bottomBar = {
            TestMessageInputBar(onMessageSend = { messagesState.value = messages + it })
        }) { paddingValues ->
        LazyColumn(
...
@Composable
fun TestMessageInputBar(onMessageSend: (ChatMessage.TextMessage) -> Unit) {
