...
fun DayView(
    ...
    Column(modifier = modifier.fillMaxSize()) {
        Row(
            ...
            Text(
                text = viewDate.format(DateTimeFormatter.ofPattern("EEEE d MMMM yyyy", Locale.getDefault())),
                style = MaterialTheme.typography.titleMedium,
                color = if (viewDate == today && !isBookingView) {
        ...
        TimeGrid(
            ...
            numberOfColumns = 1
        ) { hour, _, contentModifier ->
            Box(modifier = contentModifier) {
                timeSlots.forEach { request ->
                    val startTime = request.meetingDate?.toInstant()?.atZone(ZoneId.systemDefault())?.toLocalTime() ?: return@forEach
