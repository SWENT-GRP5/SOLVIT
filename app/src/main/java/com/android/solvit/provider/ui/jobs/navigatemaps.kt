...
fun navigateToAllSortedJobs(context: Context, currentLocation: LatLng, jobs: List<Job>) {
  if (jobs.isEmpty()) {
  ...
  val sortedJobs = jobs.sortedWith(compareBy({ it.date }, { it.time }))
  ...
  val waypoints =
      sortedJobs
          .dropLast(1)
          .mapNotNull { job -> job.location!!.let { "${it.latitude},${it.longitude}" } }
          .joinToString("|")
