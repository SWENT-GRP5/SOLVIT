...
class EditPreferencesTest {
  ...
  fun setUp() {
    ...
    val selectedPreferences =
        listOf(
            ...
            "🎉 Event Planning",
            "💇 Hair Styling")
    `when`(userRepository.getUserPreferences(eq(userId), any(), any())).thenAnswer { invocation ->
      // Get the callback for onSuccess (assuming getUserPreferences uses a callback or a similar
      // approach)
  ...
  fun testDisplayAllComponents() {
    ...
    composeTestRule
        .onNodeWithTag("selected_preferences_title")
        .assertTextEquals("Selected preferences:")
    composeTestRule.onNodeWithTag("available_preferences_title").assertTextEquals("You might like:")
